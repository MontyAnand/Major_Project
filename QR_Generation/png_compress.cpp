...
void write_png(const char *filename, unsigned char *data, int width, int height) {
    ...
    if (!fp) throw std::runtime_error("Failed to open file for writing PNG");
...
std::string file_to_base64(const std::string &filename) {
    ...
    if (valb > -6) oss << encode_table[((val << 8) >> (valb + 8)) & 0x3F];
    while (oss.str().size() % 4) oss << '=';
    return oss.str();
}
...
std::string generate_qr_base64(const std::string &text, int scale = 10) {
    ...
    if (!qrcode) throw std::runtime_error("Failed to encode QR code");
...
int main() {
    try {
        std::string qr_text = "https://example.com";
        std::string base64_encoded_qr = generate_qr_base64(qr_text);
	std::cout<<base64_encoded_qr.length()<<std::endl;
        std::cout << "Base64 Encoded QR Code PNG:\n" << "data:image/png;base64," <<base64_encoded_qr << std::endl;
